...
- The folder `abis` is the place where the Json files (generated during the compilation process based on the contracts) are. This folder and its content appear only after `truffle compile`.
